...
def run(): 
	...
	for hidden,epochs in zip(hidden_layers,epochs_pre):
		...
		inputs = Input(batch_shape=(batch_size,timesteps, x_train_temp.shape[2]))
		encoded = CuDNNLSTM(hidden,batch_input_shape=( batch_size,timesteps, x_train_temp.shape[2]),stateful = False)(inputs)
	...
	# save trained model
	model.save('2layer_0.3.h5')

	# redefine the model in order to test with one sample at a time (batch_size = 1)
	new_model = Sequential()
	new_model.add(CuDNNLSTM(hidden_layers[0],batch_input_shape=( 1,timesteps, input_dim),stateful = False))
