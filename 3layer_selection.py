...
space = {'seq_len':hp.choice('seq_len',[5,10,15,20,25,30]),
         ...
         'epochs_pre3':hp.choice('epochs_pre3',[i for i in range(50,1000)]),
         ...
         'units3':hp.choice('units3',[i for i in range(1,50)]),
...
def build_model(params):
	...
	hidden_layers = [params['units1'],params['units2'],params['units3']]
	epochs_pre = [params['epochs_pre1'],params['epochs_pre2'],params['epochs_pre3']]
	trained_encoder = []
	...
	new_model.add(CuDNNLSTM(params['units1'],batch_input_shape=( 1,timesteps, input_dim),stateful = False))
...
trials = Trials()
best = fmin(build_model, space, algo=tpe.suggest, max_evals=50, trials=trials)
print ('best: ')
print (best)
